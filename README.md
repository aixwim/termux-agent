...
- **HTTP API**: `--serve` runs a tiny local server (`POST /chat`, `GET /health`, `GET /models`, `GET /sessions`, with CORS enabled for browser clients). Every request is saved as a session and the id is returned in the response; pass `"session": "<id>"` to resume that conversation, or `"stream": true` for Server-Sent Event output. Use `--token` to require a bearer token on all endpoints except `/health`. `--serve-workers N` caps concurrent requests (0 = unlimited). OpenAI-compatible `POST /v1/chat/completions` and `GET /v1/models` let standard OpenAI SDK clients/curl talk to the agent (supports `messages`, `stream`, and `data:` image URLs; `max_tokens` maps to the context cap, the `/chat` overrides also apply, streaming responses now record a session and include it in the final chunk, and a `note` field is honored on both the streaming and non-streaming paths). `--max-context-tokens N` sets a server-wide context cap that `/chat`, `/batch`, and `/v1/chat/completions` use unless a request overrides it.
...
- **Device awareness & introspection**: `--context` injects battery/wifi/time into the system prompt (needs termux-api); `--doctor --doctor-termux` verifies which termux-api commands are installed; `--list-tools` lists registered tools; `--list-providers --json`, `--list-agents --json`, and `--models --json` give machine-readable introspection (or `--output FILE` to save any of these); `--config-show` prints the effective merged config (`--output FILE` saves it); `--config-set KEY VALUE` sets and saves a value (dot paths navigate nested keys, e.g. `providers.zen.model`), and `--config-unset KEY` removes one; `--prune --json` reports what was deleted. In the REPL, `/memory` shows (or clears) the persistent memory.
...
- **Review & scripting**: `--show [SESSION]` prints a full transcript (plus a rough token estimate, `--redact` masks secrets, `--last N` shows only the final N messages); `--summarize [SESSION]` distills a conversation via the agent (`--notify` supported, `--redact` masks secrets before sending, `--temperature` overrides sampling; `--summarize --all` summarizes every session into one report); `--rerun [SESSION]` re-runs the session's last question with the current model (`--diff` compares against the old answer, `--notify` sends a Termux notification, `--redact` masks secrets, `--temperature` overrides sampling, and `--append` adds the answer to `--output` instead of overwriting; `--rerun --all` re-runs every session's last question); `--export --markdown` writes a readable transcript; `--export-all --markdown` dumps every session as `.md`; `--tokens FILE` estimates token usage (`--session ID` for a session transcript, a directory counts all text files in the tree, `--tokens-exclude PATTERN` skips matching files in a directory scan, `--tokens HEAD`/`HEAD~N` estimates your uncommitted/working-tree git diff with a `files` count, `--tokens --all` totals every saved session with a per-session breakdown, `--output FILE` saves it); `--tokens --json` returns a structured count (with a `files` field for directories); `--no-save` keeps one-shot runs out of the session store; `--no-memory` ignores saved notes; `--session-dir DIR` uses an alternate session store; `--git` injects repo state into the system prompt; `--log FILE` writes a timestamped JSONL run log (one-shot, batch, watch, and REPL turns); `--batch --workers N` runs prompts in parallel (read from stdin with `--batch -`, pass a directory to batch every `.txt`/`.md` file in it, attach files with `--attach FILE`, `--temperature` overrides sampling) and `--fail-fast` aborts at the first error; `--forget --json` reports deletions; `--note SESSION "TEXT"` attaches a note to a session (`--note SESSION` prints it, `--note --clear` removes it, `--note --list` shows all), notes appear in `--show`, `--sessions`, and bundle/restore, and are cleaned up by `--forget`/`--prune`/`--prune-days`; `--bundle`/`--restore` back up and restore config+memory+sessions (`-` streams/pulls a gzipped tar via stdin/stdout, so `termux-agent --bundle - > b.tgz` and `cat b.tgz | termux-agent --restore -`; `--bundle --json` prints a manifest, `--bundle --no-sessions` skips session files, `--restore --dry-run` previews, `--restore --merge` keeps existing files); `--cron` prints a ready-to-add cron line (`--cron --json` returns it structured, `--cron --notify` adds a notification, `--output FILE` saves it); `--no-color` disables ANSI; `--prune`/`--prune-days` accept `--dry-run` to preview deletions (`--prune-days --keep N` keeps the N newest); `--sessions --since-days N` only lists sessions from the last N days; `--sessions --notes-only` lists only sessions that have a note; `--search-sessions TERM` scans every transcript (and note) and lists matching sessions with a snippet (`--json`/`--output` supported, exit 1 when nothing matches); `--watch` can be capped with `--max-rounds` or `--max-wait` (seconds), `--output FILE` writes the latest answer each round (`--append` adds each round instead of overwriting), `--exit-on-change` stops as soon as the answer changes, and `--exit-on-contains TEXT` stops as soon as the answer contains TEXT; `--watch --diff` skips unchanged rounds and prints a unified diff when the answer changes (all rounds honor `--temperature`); `--init --force` overwrites an existing config; `--screenshot-dir DIR` stores captures elsewhere and `--cleanup` removes leftover screenshots (`--cleanup --json` reports the count); `--stats-all` aggregates session counts, messages, characters, and per-provider/per-day breakdowns across every saved session (`--json` for structured output); The HTTP API can run detached with `--serve-background`/`--serve-stop` (pid file under `~/.termux-agent/`), read its bearer token from `--token-file`, auto-assign a port with `--port 0`, restrict the CORS origin with `--cors-origin`, serve HTTPS with `--tls-cert`/`--tls-key`, log every request as JSONL with `--log FILE`, and exposes `GET /tools`, `GET /agents`, `GET /stats`, `GET /memory`, `GET /sessions` (`?note=TERM` filters by note text, `?limit=N` caps the list), `GET /sessions/<id>` (`?markdown=1` for a readable transcript), `POST /sessions/<id>/note` (attach/update a session note) and `DELETE /sessions/<id>`; `POST /chat` accepts per-request `provider`/`model`/`rules`/`image` (path or URL)/`only_tools`/`temperature`/`max_tool_rounds`/`max_context_tokens`/`system_prompt`/`notify` overrides plus `max_output_chars`/`command_timeout`/`disabled_groups`/`retries`/`no_fallback`/`allow_dirs`/`readonly`/`memory`, and a `note` field to attach a note to the resulting session, `POST /memory` updates the persistent notes, and `POST /batch` runs a list of prompts in parallel (same overrides as `/chat`), `POST /summarize` summarizes a stored session via the agent (`{"session": id}`), and `POST /rerun` re-runs a stored session's last question (`{"session": id}` returns the new and previous answer). `GET /health` reports version, pid, and uptime; `GET /models?provider=X` lists a provider's models. `--provider zen:model` is a shorthand for setting both at once, `--no-stream` forces a non-streaming answer, `--attach FILE` reads file contents into the prompt (works for one-shot, `--resume`, `--watch`, `--batch`, and `--rerun`), `--completion SHELL` prints a completion script for bash/zsh/fish (`--output FILE` saves it), `--rotate` falls back to the next provider model on failure, `--show-system-prompt` prints the effective system prompt, and `--watch --notify` sends a Termux notification each round (`--watch --diff` only shows rounds whose answer changed).
...
- **Session backup**: `--export [ID]` prints a session as portable JSON (redirect to a file or `--output FILE`, `--redact` masks secrets), `--export-all DIR` backs up every session (`--redact` masks secrets too), `--import FILE` restores one (`-` reads stdin, `--markdown` forces Markdown parsing; JSON vs Markdown is auto-detected otherwise; pass a directory to import every `.json`/`.md` file in it, `--dry-run` validates without importing), `--prune N` / `--forget [ID]` delete sessions (`--prune`/`--forget --output FILE` save the report, `--forget --all` deletes every session and `--forget --dry-run` previews it first; `--prune`/`--prune-days` accept `--dry-run` too). `--bench [PROVIDER]` times one tiny request per model to help you pick a fast default (`--bench-prompt TEXT` uses a custom prompt, `--bench --all` benchmarks every configured provider, `--output FILE` saves the structured result).
...
- **Scripting output**: `--json` (structured result) and `--quiet` (only the answer, no banner) for pipelines; `--copy` sends the answer to the clipboard; `--stats` prints token usage after the answer (`--stats --json` always includes the `usage` object in the JSON result).
...
- **Persistent memory**: `/remember <text>` appends notes to `~/.termux-agent/memory.md`, which is loaded into every new session's instructions. `/note [TEXT]` attaches a note to just the current session (`/note` reads it, `/note clear` removes it, `/notes` lists the notes on all sessions); `GET /sessions` from the HTTP API also reports each session's note, `/tokens` estimates the current conversation's token usage, `/session` shows the current session id, `/last` re-prints the last answer, and `/export json` writes the conversation as a portable JSON file.
...
- **Auto-completion**: `--install-completion bash|zsh|fish` adds Tab-completion to your shell (providers, agents, and CLI options). `--help-json` prints the full CLI reference as machine-readable JSON for tools and docs.
- **Diagnostics**: `--doctor` checks the Termux environment, config, PATH, free disk space, session storage, and API key (`--quick` skips slow network/update checks, `--output FILE` saves the report, `--doctor-fix` repairs common issues such as a missing config file); `--health` is a faster offline subset (version, config, working dir, provider, session storage); `--doctor-network` also tests provider connectivity; `--smoke` sends a tiny real prompt end-to-end to verify the whole pipeline (`--smoke --json` returns a structured result, `--timeout` caps the wait, `--output FILE` saves it).
...
- **Session search**: `/search TERM` in the REPL finds sessions whose transcript contains the term. `/attach FILE` (or a URL) reads content into the next turn, `/retry` re-runs the last turn, `/redo` re-runs the last turn with the current provider/model after rolling back the previous answer, `/quiet` toggles streaming (answer prints when done), `/temp N` sets sampling temperature, `/maxrounds N` the tool-round limit, `/usage` shows token usage, and `/bench` runs a quick latency benchmark.
- **Session backup**: `--export [ID]` prints a session as portable JSON (redirect to a file), `--export-all DIR` backs up every session (or `--export-all FILE --json` writes one combined JSON), `--import FILE` restores one (`-` reads stdin, `--json` reports the new id), `--prune N` / `--forget [ID]` delete sessions. `--bundle -` streams a gzipped tar of config+memory+sessions to stdout (`--bundle --no-sessions` leaves sessions out), and `--sessions --all` lists every session instead of the default 20.
...
# install Tab-completion (bash/zsh/fish) once
...
termux-agent --bundle ./backup-nosessions --no-sessions   # config+memory only
...
termux-agent --sessions --all       # every session, ignoring --limit
...
/provider NAME switch provider (e.g. /provider ollama, /provider zen:model-name)
