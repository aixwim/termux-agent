...
- **Persistent memory**: `/remember <text>` appends notes to `~/.termux-agent/memory.md`, which is loaded into every new session's instructions. `/note [TEXT]` attaches a note to just the current session (`/note` reads it, `/note clear` removes it); `GET /sessions` from the HTTP API also reports each session's note.
