...
- **Diagnostics**: `--doctor` checks the Termux environment, config, PATH, free disk space, and API key; `--doctor-network` also tests provider connectivity; `--smoke` sends a tiny real prompt end-to-end to verify the whole pipeline (`--smoke --json` returns a structured result).
...
- **Session backup**: `--export [ID]` prints a session as portable JSON (redirect to a file), `--export-all DIR` backs up every session, `--import FILE` restores one (`-` reads stdin, `--json` reports the new id), `--prune N` / `--forget [ID]` delete sessions. `--bundle -` streams a gzipped tar of config+memory+sessions to stdout.
