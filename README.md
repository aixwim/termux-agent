...
- **Diagnostics**: `--doctor` checks the Termux environment, config, PATH, free disk space, session storage, and API key (`--quick` skips slow network/update checks, `--output FILE` saves the report, `--doctor-fix` repairs common issues such as a missing config file); `--doctor-network` also tests provider connectivity; `--smoke` sends a tiny real prompt end-to-end to verify the whole pipeline (`--smoke --json` returns a structured result, `--timeout` caps the wait, `--output FILE` saves it).
