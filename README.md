...
- **Diagnostics**: `--doctor` checks the Termux environment, config, PATH, free disk space, session storage, and API key; `--doctor-network` also tests provider connectivity; `--smoke` sends a tiny real prompt end-to-end to verify the whole pipeline (`--smoke --json` returns a structured result, `--output FILE` saves it).
