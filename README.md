...
- **Session backup**: `--export [ID]` prints a session as portable JSON (redirect to a file, `--redact` masks secrets), `--export-all DIR` backs up every session (`--redact` masks secrets too), `--import FILE` restores one (`-` reads stdin), `--prune N` / `--forget [ID]` delete sessions. `--bench [PROVIDER]` times one tiny request per model to help you pick a fast default (`--output FILE` saves the structured result).
...
- **Diagnostics**: `--doctor` checks the Termux environment, config, PATH, free disk space, session storage, and API key (`--quick` skips slow network/update checks); `--doctor-network` also tests provider connectivity; `--smoke` sends a tiny real prompt end-to-end to verify the whole pipeline (`--smoke --json` returns a structured result, `--output FILE` saves it).
