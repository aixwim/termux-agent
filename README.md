...
- **Session backup**: `--export [ID]` prints a session as portable JSON (redirect to a file or `--output FILE`, `--redact` masks secrets), `--export-all DIR` backs up every session (`--redact` masks secrets too), `--import FILE` restores one (`-` reads stdin), `--prune N` / `--forget [ID]` delete sessions (`--prune --output FILE` saves the report). `--bench [PROVIDER]` times one tiny request per model to help you pick a fast default (`--output FILE` saves the structured result).
